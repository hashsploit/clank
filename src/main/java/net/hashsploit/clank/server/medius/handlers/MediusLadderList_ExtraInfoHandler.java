...
public class MediusLadderList_ExtraInfoHandler extends MediusPacketHandler {
	...
	public List<MediusMessage> write(MediusClient client) {
		...
		byte[] accountName = Utils.buildByteArrayFromString("Test", MediusConstants.ACCOUNTNAME_MAXLEN.getValue());
		byte[] accountStats = Utils.buildByteArrayFromString("", MediusConstants.ACCOUNTSTATS_MAXLEN.getValue());
		byte[] onlineStateFields = Utils.hexStringToByteArray("00000000" + "00000000" + "00000000");
		byte[] onlineStateLobbyName = Utils.buildByteArrayFromString("", MediusConstants.WORLDNAME_MAXLEN.getValue());
		byte[] onlineStateGameName = Utils.buildByteArrayFromString("", MediusConstants.WORLDNAME_MAXLEN.getValue());
