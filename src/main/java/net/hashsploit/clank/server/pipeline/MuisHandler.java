...
public class MuisHandler extends MessageToMessageDecoder<ByteBuf> {

	private static final Logger logger = Logger.getLogger(MuisHandler.class.getName());
	private final MediusClient client;

	public MuisHandler(final MediusClient client) {
		super();
		this.client = client;
	}
	...
	protected void decode(ChannelHandlerContext ctx, ByteBuf msg, List<Object> out) throws Exception {
		logger.finest("MUIS HANDLER IN [Client: " + client.getIP() + "]: " + Utils.bytesToHex(Utils.nettyByteBufToByteArray(msg)));
		...
		if (messages != null && messages.size() > 0) {
			...
			for (final RTMessage message : messages) {
				...
				logger.finest("MUIS HANDLER OUT: " + Utils.bytesToHex(Utils.nettyByteBufToByteArray(message.getFullMessage())));
