...
public class DmeTcpHandler extends MessageToMessageDecoder<ByteBuf> {

	private static final Logger logger = Logger.getLogger(DmeTcpHandler.class.getName());
	private final MediusClient client;

	public DmeTcpHandler(final MediusClient client) {
		super();
		this.client = client;
	}
	...
	protected void decode(ChannelHandlerContext ctx, ByteBuf msg, List<Object> out) throws Exception {
		...
		if (messages != null && messages.size() > 0) {
			...
			for (final RTMessage message : messages) {
				...
				logger.finest("MLS HANDLER OUT: " + Utils.bytesToHex(Utils.nettyByteBufToByteArray(message.getFullMessage())));
